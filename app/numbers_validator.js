...
/**
 *
 * A simple classcontaining methods for validating
...
class NumbersValidator {
  ...
  isNumberEven(number) {
    ...
    if (typeOfVariable !== `number`) {
      ...
      throw new Error(`[${number}] is not of type "Number" it is of type "${typeOfVeriable}"`);
    } else {
      return number % 2 === 0;
    }
  }
   ...
   *
   *
   * @param {Array<Numbers>} arrayOfNumbers array of numbers to check
  ...
  getEvenNumbersFromArray(arrayOfNumbers) {
    ...
    arrayOfNumbers.every((item) => typeof item === `number`)) {
      ...
      throw new Error(`${arratOfNumbers} is not an array of "Numbers"`);
    }
  }
}
